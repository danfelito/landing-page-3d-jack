...
function HeroSection() {
  return <section className="relative flex h-screen flex-col overflow-x-clip bg-ink">
    <FadeIn y={-20}><nav className="flex justify-between px-6 pt-6 text-sm font-medium uppercase tracking-wider text-ice md:px-10 md:pt-8 md:text-lg lg:text-[1.4rem]">{['About', 'Price', 'Projects', 'Contact'].map((item) => <a key={item} href={`#${item.toLowerCase()}`} className="transition duration-200 hover:opacity-70">{item}</a>)}</nav></FadeIn>
    <FadeIn delay={0.15} y={40} className="overflow-hidden"><h1 className="hero-heading mt-6 w-full whitespace-nowrap text-center text-[14vw] font-black uppercase leading-none tracking-tight sm:mt-4 sm:text-[15vw] md:-mt-5 md:text-[16vw] lg:text-[17.5vw]">Hi, i&apos;m jack</h1></FadeIn>
    <FadeIn delay={0.6} y={30} className="absolute left-1/2 top-1/2 z-10 w-[280px] -translate-x-1/2 -translate-y-1/2 sm:bottom-0 sm:top-auto sm:w-[360px] sm:translate-y-0 md:w-[440px] lg:w-[520px]"><Magnet><img src={robotPortrait} alt="Robot portrait" className="w-full object-contain drop-shadow-[0_0_70px_rgba(182,0,168,0.35)]" /></Magnet></FadeIn>
    <div className="relative z-20 mt-auto flex items-end justify-between px-6 pb-7 sm:pb-8 md:px-10 md:pb-10"><FadeIn delay={0.35} y={20}><p className="max-w-[160px] text-[clamp(0.75rem,1.4vw,1.5rem)] font-light uppercase leading-snug tracking-wide text-ice sm:max-w-[220px] md:max-w-[260px]">a 3d creator driven by crafting striking and unforgettable projects</p></FadeIn><FadeIn delay={0.5} y={20}><ContactButton /></FadeIn></div>
  </section>;
}
...
function MarqueeSection() {
  ...
  const row = (imgs: string[], right = true) => <div className="flex gap-3" style={{ transform: `translateX(${right ? offset - 200 : -(offset - 200)}px)`, willChange: 'transform' }}>{[...imgs, ...imgs, ...imgs].map((src, i) => <img key={`${src}-${i}`} src={src} alt="3D project preview" loading="lazy" className="h-[270px] w-[420px] shrink-0 rounded-2xl object-cover" />)}</div>;
...
function AboutSection() {
  ...
  return <section id="about" className="relative flex min-h-screen items-center justify-center overflow-hidden bg-ink px-5 py-20 sm:px-8 md:px-10">
    {decor.map(([src, pos], i) => <FadeIn key={src} delay={0.1 + i * 0.06} x={i < 2 ? -80 : 80} y={0} className={`absolute ${pos}`}><img src={src} alt="3D decorative object" /></FadeIn>)}
    <div className="relative z-10 flex flex-col items-center gap-12 md:gap-16"><FadeIn><h2 className="hero-heading text-center text-[clamp(3rem,12vw,160px)] font-black uppercase leading-none tracking-tight">About me</h2></FadeIn><motion.p ref={ref} style={{ opacity }} className="mx-auto max-w-[560px] text-center text-[clamp(1rem,2vw,1.35rem)] font-medium leading-relaxed text-ice">With more than five years of experience in design, i focus on branding, web design, and user experience, i truly enjoy working with businesses that aim to stand out and present their best image. Let&apos;s build something incredible together!</motion.p><ContactButton /></div>
  </section>;
}

function ServicesSection() {
  return <section id="price" className="rounded-t-[40px] bg-white px-5 py-20 text-ink sm:rounded-t-[50px] sm:px-8 sm:py-24 md:rounded-t-[60px] md:px-10 md:py-32"><FadeIn><h2 className="mb-16 text-center text-[clamp(3rem,12vw,160px)] font-black uppercase leading-none md:mb-28">Services</h2></FadeIn><div className="mx-auto max-w-5xl">{services.map(([n, t, d], i) => <FadeIn key={n} delay={i * 0.1}><div className="grid grid-cols-[0.42fr_1fr] gap-6 border-t border-black/15 py-8 last:border-b sm:py-10 md:py-12"><span className="text-[clamp(3rem,10vw,140px)] font-black leading-none">{n}</span><div className="flex flex-col justify-center gap-3"><h3 className="text-[clamp(1rem,2.2vw,2.1rem)] font-medium uppercase">{t}</h3><p className="max-w-2xl text-[clamp(0.85rem,1.6vw,1.25rem)] font-light leading-relaxed opacity-60">{d}</p></div></div></FadeIn>)}</div></section>;
}
...
function ProjectCard({ project, index }: { project: (typeof projects)[number]; index: number }) {
  ...
  return <div ref={ref} className="h-[85vh]"><motion.article className="sticky rounded-[40px] border-2 border-ice bg-ink p-4 sm:rounded-[50px] sm:p-6 md:rounded-[60px] md:p-8" style={{ top: `calc(6rem + ${index * 28}px)`, scale }}><div className="mb-6 flex flex-col gap-5 text-ice lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap items-end gap-5"><span className="text-[clamp(3rem,10vw,140px)] font-black leading-none">{number}</span><div className="pb-3"><p className="text-sm uppercase tracking-[0.35em] opacity-60">{category}</p><h3 className="text-[clamp(1.6rem,4vw,4rem)] font-black uppercase leading-none">{name}</h3></div></div><button className="inline-flex items-center gap-2 rounded-full border-2 border-ice px-6 py-2.5 text-xs font-medium uppercase tracking-widest transition hover:bg-ice/10 sm:px-10 sm:py-3.5">Live Project <ArrowUpRight size={18} /></button></div><div className="grid gap-3 lg:grid-cols-[0.4fr_0.6fr]"><div className="grid gap-3"><img src={images[0]} alt="Project preview" className="h-[clamp(130px,16vw,230px)] w-full rounded-[40px] object-cover sm:rounded-[50px] md:rounded-[60px]" /><img src={images[1]} alt="Project preview" className="h-[clamp(160px,22vw,340px)] w-full rounded-[40px] object-cover sm:rounded-[50px] md:rounded-[60px]" /></div><img src={images[2]} alt="Project preview" className="h-full min-h-[320px] w-full rounded-[40px] object-cover sm:rounded-[50px] md:rounded-[60px]" /></div></motion.article></div>;
}

function ProjectsSection() {
  return <section id="projects" className="relative z-10 -mt-10 rounded-t-[40px] bg-ink px-5 py-20 sm:-mt-12 sm:rounded-t-[50px] sm:px-8 md:-mt-14 md:rounded-t-[60px] md:px-10"><FadeIn><h2 className="hero-heading mb-16 text-center text-[clamp(3rem,12vw,160px)] font-black uppercase leading-none tracking-tight">Project</h2></FadeIn><div className="mx-auto max-w-7xl">{projects.map((project, index) => <ProjectCard key={project[1]} project={project} index={index} />)}</div></section>;
}
